...
void connection::send(context_ptr& ctx)
{
    auto self = this->shared_from_this();
    ios_wrapper_.write(self, ctx);
}

void connection::on_error(const boost::system::error_code& error)
{
    close();
...
const req_header& connection::get_read_header() const
{
    return head_;
}
...
{
    if(head_.len > MAX_BUFF_LEN)
    {
        socket_.close();
        return;
    }

    if(head_.len > MAX_PAGE_SIZE)
    {
...
void connection::handle_read_head(const boost::system::error_code& error)
{
    if(!socket_.is_open())
    {
        return;
...
void connection::handle_time_out(const boost::system::error_code& error)
{
    if(!socket_.is_open())
    {
        return;
